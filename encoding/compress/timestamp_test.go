...
func TestTimeArrayEncodeAll(t *testing.T) {
	...
	buf := &bytes.Buffer{}
	_, _, err := TimeArrayEncodeAll(src, buf)
	b := buf.Bytes()
	if err != nil {
...
func testTimeArrayEncodeAll_Compare(t *testing.T, input []int64, encoding byte) {
	...
	buf := &bytes.Buffer{}
	min, max, err := TimeArrayEncodeAll(input, buf)
	buf2 := buf.Bytes()
	if err != nil {
	...
	expmin, expmax := vec.Int64Slice(exp).MinMax()
	if min != expmin {
		t.Fatalf("got min %v, expected %v for enc type %d", min, expmin, encoding)
	}
	if max != expmax {
		t.Fatalf("got max %v, expected %v for enc type %d", max, expmax, encoding)
	}
...
func TestTimeArrayEncodeAll_NoValues(t *testing.T) {
	buf := &bytes.Buffer{}
	_, _, err := TimeArrayEncodeAll(nil, buf)
	b := buf.Bytes()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestTimeArrayEncodeAll_Quick(t *testing.T) {
	quick.Check(func(values []int64) bool {
		...
		buf := &bytes.Buffer{}
		_, _, err := TimeArrayEncodeAll(values, buf)
		b := buf.Bytes()
		if err != nil {
			t.Fatal(err)
		}
	...
	for _, n := range cases {
		...
		b.Run(fmt.Sprintf("%d_seq", n), func(b *testing.B) {
			...
			b.Run("batch", func(b *testing.B) {
				...
				b.ResetTimer()
				for n := 0; n < b.N; n++ {
					if _, _, err = TimeArrayEncodeAll(input, bufResultBuffer); err != nil {
						b.Fatal(err)
					}
