...
var (
	...
	uint64NotEqualTestCount_1  int64  = 22
	...
	uint64NotEqualTestCount_2  int64  = 24
...
var uint64NotEqualCases = []Uint64MatchTest{
	Uint64MatchTest{
		...
		result: uint64NotEqualTestResult_1,
		count:  uint64NotEqualTestCount_1,
	}, {
		name:   "l64",
		slice:  append(uint64TestSlice_1, uint64TestSlice_1...),
		match:  uint64NotEqualTestMatch_1,
		result: append(uint64NotEqualTestResult_1, uint64NotEqualTestResult_1...),
		count:  uint64NotEqualTestCount_1 * 2,
	}, {
		name:   "l31",
		slice:  uint64TestSlice_1[:31],
		match:  uint64NotEqualTestMatch_1,
		result: []byte{0x7d, 0xbd, 0xdc, 0x8e},
		count:  uint64NotEqualTestCount_1 - 1,
	}, {
		name:   "l23",
		slice:  uint64TestSlice_1[:23],
		match:  uint64NotEqualTestMatch_1,
		result: []byte{0x7d, 0xbd, 0xdc}, // last match is gone
		count:  uint64NotEqualTestCount_1 - 5,
	}, {
		name:   "l15",
		slice:  uint64TestSlice_1[:15],
		match:  uint64NotEqualTestMatch_1,
		result: []byte{0x7d, 0xbc},
		count:  uint64NotEqualTestCount_1 - 11,
	}, {
		name:   "l7",
		slice:  uint64TestSlice_1[:7],
		match:  uint64NotEqualTestMatch_1,
		result: []byte{0x7c},
		count:  uint64NotEqualTestCount_1 - 17,
	}, {
		name:   "l0",
		slice:  make([]uint64, 0),
		match:  uint64NotEqualTestMatch_1,
		...
		name:   "ext32",
		slice:  uint64TestSlice_2,
		match:  uint64NotEqualTestMatch_2,
		result: uint64NotEqualTestResult_2,
		count:  uint64NotEqualTestCount_2,
	}, {
		// with extreme values, test scalar algorithm
		name:   "ext31",
		slice:  uint64TestSlice_2[:31],
		match:  uint64NotEqualTestMatch_2,
		result: []byte{0xee, 0xee, 0xee, 0xee},
		count:  uint64NotEqualTestCount_2,
	},
}
...
func BenchmarkMatchUint64NotEqualGeneric(B *testing.B) {
	for _, n := range []int{32, 128, 1024, 4096, 64 * 1024, 128 * 1024} {
		B.Run(fmt.Sprintf("%d", n), func(B *testing.B) {
			...
			B.SetBytes(int64(n * 8))
			for i := 0; i < B.N; i++ {
				matchUint64NotEqualGeneric(a, 5, bits)
			}
		})
	}
}
...
func BenchmarkMatchUint64NotEqualAVX2(B *testing.B) {
	for _, n := range []int{32, 128, 1024, 4096, 64 * 1024, 128 * 1024} {
		B.Run(fmt.Sprintf("%d", n), func(B *testing.B) {
			...
			B.SetBytes(int64(n * 8))
			for i := 0; i < B.N; i++ {
				matchUint64NotEqualAVX2(a, 5, bits)
			}
		})
	}
}
