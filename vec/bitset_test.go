...
func TestBitSetNew(T *testing.T) {
	for _, c := range bitSetCases {
		T.Run(c.name, func(t *testing.T) {
			...
			if got, want := int(bits.Count()), 0; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
			}
...
func TestBitSetFromBytes(T *testing.T) {
	for _, c := range bitSetCases {
		T.Run(c.name, func(t *testing.T) {
			...
			if got, want := int(bits.Count()), c.count; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
			}
...
func TestBitSetOne(T *testing.T) {
	for _, sz := range bitSetSizes {
		T.Run(f("%d", sz), func(t *testing.T) {
			...
			if got, want := int(bits.Count()), sz; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
			}
...
func TestBitSetResize(T *testing.T) {
	for _, sz := range bitSetSizes {
		for _, sznew := range bitSetSizes {
			T.Run(f("%d_%d", sz, sznew), func(t *testing.T) {
				...
				if got, want := int(bits.Count()), min(sz, sznew); got != want {
					T.Errorf("unexpected count %d, expected %d", got, want)
					T.FailNow()
				}
				if got, want := int(bits.Count()), popcount(bits.Bytes()); got != want {
	...
	for _, sz := range bitSetSizes {
		T.Run(f("%d_grow_0", sz), func(t *testing.T) {
			...
			if got, want := bits.Size(), 0; got != want {
				T.Errorf("unexpected size %d, expected %d", got, want)
				T.FailNow()
			}
			if got, want := int(bits.Count()), 0; got != want {
	...
	for _, sz := range bitSetSizes {
		T.Run(f("%d_grow+1", sz), func(t *testing.T) {
			...
			if got, want := bits.Size(), sz+1; got != want {
				T.Errorf("unexpected size %d, expected %d", got, want)
				T.FailNow()
			}
			if got, want := int(bits.Count()), sz+1; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
				T.FailNow()
			}
			if got, want := int(bits.Count()), popcount(bits.Bytes()); got != want {
	...
	for _, sz := range bitSetSizes {
		for _, pt := range bitSetPatterns {
			T.Run(f("%d_%x", sz, pt), func(t *testing.T) {
				...
				if got, want := int(bits.Count()), popcount(cmp); got != want {
					T.Errorf("unexpected count %d, expected %d", got, want)
				}
...
func TestBitSetSet(T *testing.T) {
	for _, sz := range bitSetSizes {
		T.Run(f("%d", sz), func(t *testing.T) {
			...
			bits.Set(0)
			cmp[0] |= 0x80
			if got, want := int(bits.Count()), 1; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
			}
			if !bits.IsSet(0) {
			...
			if got, want := int(bits.Count()), 2; got != want {
				T.Errorf("unexpected count %d, expected %d", got, want)
			}
			if !bits.IsSet(sz - 1) {
				T.Errorf("unexpected IsSet=false")
