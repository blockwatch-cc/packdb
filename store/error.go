...
const (
	...
	// ErrTxConflict is returned when a transaction conflicts with another
	// transaction. This can happen if a read key had been updated concurrently
...
func IsError(err error, c ErrorCode) bool {
	switch e := err.(type) {
	case Error:
